   ...
   pageload();
    ...
    function pageload(){
        ...
        $.ajax({
                ...
                method     : 'GET',
                url        : '<?php echo site_url('admin/index.php/Home/home_list');?>',
                dataType   : 'json',
                        ...
                        table.clear();
                          ...
                          $.each(res, function(i, item) {
                               ...
                               let  image_name   = res[i]['image_name'],
                                    image        = `<img src="<?php echo base_url('admin/writable/homeLogo');?>/${res[i]['image_name']}" width="50" height="50" id="img1">`,
                                                                ...
                                                                <div class="dropdown">
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-right">
                                                                        <ul class="link-list-opt no-bdr">
                                                                           <li id="edit-logo" ids="${res[i]['id']}"><a href="javascript:void(0)"><em class="icon ni ni-edit"></em><span>Edit Logo</span></a></li>
                                                                            <li id="delete-logo" ids="${res[i]['id']}"><a href="javascript:void(0)"><em class="icon ni ni-delete"></em><span>Delete Logo</span></a></li>
    ...
    $(document).on('click','#home-modal',function(e){
        $('#logoAdd').modal('show'); 
    })
...
$(document).ready(function () {
            $('#home_logo').on('submit', function (e) {

                e.preventDefault();
                if ($('#file').val() == '') {
                    $('.imgUploadButton').html('Uploading ...');
                    $('.imgUploadButton').prop('enabled');
                    document.getElementById("home_logo").reset();
                } else {
                    $.ajax({
                        url: "<?php echo site_url('admin/index.php/logo_upload'); ?>",
                        method: "POST",
    ...
    $(document).on('click','#change-status',function(e){
        ...
        var status = $(this).attr('status');
          $.ajax({
                method: "POST",  
                url : '<?php echo site_url('admin/index.php/Home/changeLogoStatus');?>',
                data : {'ids' : ids ,
                        'status':status},
                dataType : 'json',
...
$(document).on('click','#delete-logo',function(e){   //single delete
    ...
    swal.fire({
            ...
            if (result.isConfirmed) {
              $.ajax({
                method: "POST",  
                url : '<?php echo site_url('admin/index.php/Home/deleteLogo');?>',
                data : {'ids' : ids },
                dataType : 'json',
                success: function(data)
                {
...
function deleteAll(event) 
{
    ...
    var rows = $('#home_table').find('tbody tr');
    $.each(rows, function() {
