...
class Controller
{
    ...
    public function admin_layout($data)     //side bar
    {    
      //  print_r($data);die();
        $loginModel = new Login_Model();
        $parent_id      = 0;
        $url_name       = $this->request->uri->getSegments();
        $url            = " ";
        

        foreach ($url_name as $key => $value) {
            if(array_key_exists($key+1,$url_name))
            {
                $url.= $value.'/';
            }
            else
        ...
        $data['url_parameter']  = $url;
        $parent = $loginModel->fetch_menus($parent_id);
        for ($i = 0; $i < sizeof($parent); $i++) 
        {
            $child_menus            = $loginModel->fetch_menus($parent[$i]['id']);
            $parent[$i]['child']    = $child_menus;
            $data['navbar']         = $parent;
        }
